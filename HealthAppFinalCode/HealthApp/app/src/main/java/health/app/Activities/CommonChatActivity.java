...
public class CommonChatActivity extends BaseActivity implements SwipeRefreshLayout.OnRefreshListener {
    public TrainerDashboardListResponse.ApprovedData messagetoClient;
    public MyTrainerListResponse.MyTrainerData trainerData;
    public MyTrainerListResponse.MyTrainerData messagetoTrainer;
    ...
    public static final String MESSAGETRAINER = "messageTrainer";
    public static final String MESSAGECLIENT = "messageClient";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        messagetoTrainer=(MyTrainerListResponse.MyTrainerData) getIntent().getParcelableExtra(MESSAGETRAINER);
        ...
        messagetoClient = (TrainerDashboardListResponse.ApprovedData) getIntent().getParcelableExtra(MESSAGECLIENT);
        customerId1=getIntent().getStringExtra("customerId");
        ...
        if (messagetoClient!=null) {
            title.setText(messagetoClient.getFirst_name()+" "+messagetoClient.getLast_name());
        }
        else if (clientData!=null) {
            title.setText(clientData.getFirst_name()+" "+clientData.getLast_name());
        }
        else if (trainerData!=null) {
            title.setText(trainerData.getFirst_name()+" "+trainerData.getLast_name());
        }
        else if (messagetoTrainer!=null) {
            title.setText(messagetoTrainer.getFirst_name()+" "+messagetoTrainer.getLast_name());
        }
        else {
            title.setText("Chat");
        }
    ...
    public void sendNewMessage() {
        if (Preferences.getInstance().getUserType().equalsIgnoreCase("Trainer")&& clientData!=null)
        {
            if (clientData.getCustomerId()!=null) {
                receiverId = clientData.getCustomerId();
            }
        }
        else if (Preferences.getInstance().getUserType().equalsIgnoreCase("Trainer")&& messagetoClient!=null)
        {
            if (messagetoClient.getCustomerId()!=null) {
                receiverId = messagetoClient.getCustomerId();
            }
        }
        else if (Preferences.getInstance().getUserType().equalsIgnoreCase("Trainer")&& customerId1!=null)
        {
            receiverId = customerId1;
        }
        else if (Preferences.getInstance().getUserType().equalsIgnoreCase("Customer") && trainerData!=null) {
            if (trainerData.getTrainerId() != null) {
                receiverId = trainerData.getTrainerId();
            }
        }
        else if (Preferences.getInstance().getUserType().equalsIgnoreCase("Customer") && messagetoTrainer!=null )
        {
            if (messagetoTrainer.getTrainerId() != null) {
                receiverId = messagetoTrainer.getTrainerId();
            }
        }
        ...
        healthAppService.sendMessage(Preferences.getInstance().getUserId(),receiverId,toServerUnicodeEncoded).
    ...
    private void getAllUser(final String userId) {
        ...
        swipeRefreshLayout.setRefreshing(true);
        healthAppService.getChatList(userId,receiverId,"1").enqueue(new Callback<ChatListResponse>() {
            @Override
                ...
                if (response.body() != null && response.body().isStatus()) {
                    ...
                    for(int i=0;i<response.body().getChatList().size();i++){
                        if (response.body().getChatList().get(i).getMsgFrom().equals(userId)) {
                            ...
                            chatAdapter = new ChatAdapter(chat_array, CommonChatActivity.this);
    ...
    private void getAllUser1(final String userId) {
        ...
        showProgressbar("Loading", "Please wait...");
        swipeRefreshLayout.setRefreshing(true);
        healthAppService.getChatList(userId,receiverId,Next_Page).enqueue(new Callback<ChatListResponse>() {
            @Override
            public void onResponse(Call<ChatListResponse> call, Response<ChatListResponse> response) {
                if (response.body() != null && response.body().isStatus()) {
                    loading=true;
    ...
    private void alertUser(String msg) {
        final android.support.v7.app.AlertDialog.Builder alertDialogBuilder = new android.support.v7.app.AlertDialog.Builder(CommonChatActivity.this);
